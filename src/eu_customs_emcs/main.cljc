(ns eu_customs_emcs.main
  "Kotodama WASM entrypoint for the Eu_customs_emcs clean-room actor (L4) — Clojure port.
  ...
  facts (`eu_customs_emcs.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "eu_customs_emcs")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"eu_customs_emcs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "eu_customs_emcs-compat" :tier tier :entities entities} 200])
...
(defn start! [] :eu_customs_emcs-compat/ready)
