(ns eu_customs_emcs.main-test
  "Contract + behavioral test for the eu_customs_emcs-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "eu_customs_emcs-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'eu_customs_emcs.main-test)]
